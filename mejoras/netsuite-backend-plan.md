...
| BE-6 — Observabilidad: logs estructurados + LogAuditoria | ✅ Completado | 2026-06-17 | `logNetsuite` (JSON estructurado) + `auditNetsuite` no-throwing; enum `AccionAuditoria` +3 valores (migración `20260617120000_netsuite_audit_enum`, la aplica Erika); logs/audit cableados en crear/procesar/reenviar/cancelar. Commit `acf8a22`. Deuda del 500 temporal ya revertida (`17cbe64`). |
| BE-7 — Validación end-to-end en Vercel preview con FE-5 | ⏳ Pendiente | — | Coordinación con FE |
| BE-8 — `vercel.json` (runtime nodejs, maxDuration) + variables de entorno | ✅ Completado | 2026-06-17 | Docs: `docs/backend/api/netsuite-cargos-str.md` (8 endpoints + PATCH operador + payload PO + env vars + observabilidad) y `docs/runbooks/netsuite-lote-colgado.md` (lote colgado, ver errores, activar real, migraciones, runtime). `.env.example` ya documenta `NETSUITE_*`. **vercel.json NO se modifica**: `runtime`/`maxDuration` se declaran inline en cada handler (forma canónica App Router) — duplicarlos en vercel.json sería redundante. Cierra Fase 1. |
